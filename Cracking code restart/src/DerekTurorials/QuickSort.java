...
public class QuickSort { 
	...
	public void quickSort(int left, int right) {
		...
		if (right - left <= 0) return; // Everything is sorted else 
		{ 
	...
	public int partitionArray(int left, int right, int pivot) { 
		...
		swapValues(leftPointer, right); //IF left pointer meet right pointer, then swap patr_location and right
	...
	public void generateRandomArray() {
		...
		for (int i = 0; i < arraySize; i++) { // Generate a random array with values between // 10 and 59 
	...
	static void printHorzArray(int i, int j) {
		...
		if (i != -1) { // Number of spaces to put before the F 

			int spacesBeforeFront = 6 * (i + 1) - 5; 

			for (int k = 0; k < spacesBeforeFront; k++) 

				System.out.print(" "); System.out.print("L" + i); // Number of spaces to put before the R 
